...
/**
 * Rehydrate a bucket of signals from localStorage and re-serialise the whole
 * bucket whenever any leaf changes. Best-effort: storage errors are ignored.
 * @param key - Namespaced localStorage key.
...
export function persistBucket<T extends object>(key: string, bucket: T): T {
  try {
    ...
    // best-effort: ignore malformed or inaccessible storage
  }
  effect(() => {
